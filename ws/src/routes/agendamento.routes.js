...
router.post('/filter', async (req, res) => {
  try {
    const { range, clinicaId } = req.body;
...
router.post('/', async (req, res) => {
  ...
  try {
    const { clienteId, clinicaId, servicoId, colaboradorId } = req.body;

    const cliente = await Cliente.findById(clienteId).select(
      'nome endereco customerId'
    );
    const salao = await Salao.findById(clinicaId).select('recipientId');
...
router.post('/dias-disponiveis', async (req, res) => {
  try {
    const { data, clinicaId, servicoId } = req.body;
